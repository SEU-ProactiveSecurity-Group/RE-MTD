''' Attackers with a single and variable (S & V) target --  S & V Scenario '''
import gym
...
class EnvSV(gym.Env):
    ...
    def step(self, action):
        ...
        if port_list == []:
            ...
            # 攻击目标选择
            self.target = np.argmax(self.attack_state[:, 3]) # 选择权重最高的服务       
