''' Attackers with multiple and variable (M & V) targets -- M & V Scenario '''
import gym
...
class EnvMV(gym.Env):
    ...
    def step(self, action):
        ...
        if port_list == []:
            ...
            # 攻击目标选择
            target_list = []
            for port in self.attack_state[:, 0]:
                if port > 0:
            ...
            if self.attack_remain > 0:
                for port in target_list:
                    target_ser_num = self.get_state_index(port) # 在state中找到被攻击的服务序号，因为state和attack_state是通过port连接
                    target = self.get_attack_index(port)
                    attack_con = self.attack_remain * self.attack_state[target][3] // np.sum(self.attack_state[:, 3]) # 按照权重分配流量
                    if attack_con <= (self.state[target_ser_num][0] * self.pod_con_num - \
                                            self.state[target_ser_num][1]):
                        self.state[target_ser_num][1] += attack_con
                        self.attack_state[target][4] += attack_con
                        self.attack_remain -= attack_con 
                        self.attack_state[target][2] += 1
                    else:
                        self.attack_state[target][4] += self.state[target_ser_num][0] * self.pod_con_num - self.state[target_ser_num][1]
    ...
    def reset(
        ...
        *,
        ...
        for i in range(self.ser_num):
            ...
            self.state[i] = [10, connection, port] #容器数量、连接数、端口号
        ...
        if self.attack_remain > 0:
            for port in target_list:
                target_ser_num = self.get_state_index(port) # 在state中找到被攻击的服务序号，因为state和attack_state是通过port连接
                target = self.get_attack_index(port)
                attack_con = self.attack_remain * self.attack_state[i][3] // np.sum(self.attack_state[:, 3]) # 按照权重分配流量
                if attack_con <= (self.state[target_ser_num][0] * self.pod_con_num - \
                                        self.state[target_ser_num][1]):
                    self.state[target_ser_num][1] += attack_con
                    self.attack_state[target][4] += attack_con
