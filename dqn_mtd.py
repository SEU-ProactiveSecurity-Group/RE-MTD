...
from env_sc import EnvSC
from env_sv import EnvSV
from env_mc import EnvMC
from env_mv import EnvMV
...
# Add arguments
parser.add_argument('--env', type=str, required=True, help='Type of the environment')
parser.add_argument('--attacker', type=int, required=True, help='Number of attackers')
...
class DQN:
    ''' DQN算法 '''
    def __init__(self, state_dim, hidden1_dim, hidden2_dim,action_dim, learning_rate, gamma,
                 epsilon, target_update, device):
        self.state_dim = state_dim
        self.action_dim = action_dim
        self.q_net = Qnet(state_dim, hidden1_dim, hidden2_dim,
                          self.action_dim).to(device)  # Q网络
        # 目标网络
        self.target_q_net = Qnet(state_dim, hidden1_dim, hidden2_dim,
                          self.action_dim).to(device)
        # 使用Adam优化器
        self.optimizer = torch.optim.Adam(self.q_net.parameters(), betas=(0.9,0.999), lr=learning_rate)

        # 使用learning rate decay
        self.scheduler = torch.optim.lr_scheduler.StepLR(self.optimizer,step_size=40000, gamma=0.2)
  
        self.gamma = gamma  # 折扣因子
        self.epsilon = epsilon  # epsilon-贪婪策略
        self.target_update = target_update  # 目标网络更新频率
        self.sample_count = 0  # 按一定概率随机选动作，即 e-greedy 策略，并且epsilon逐渐衰减
    ...
    def update(self, transition_dict):
        states = torch.tensor(transition_dict['states'],
                              dtype=torch.float).view(-1, 30).to(self.device)
        ...
        q_values = self.q_net(states).gather(1, actions)  # Q值,gather函数用来选取state中的值
        # 下个状态的最大Q值
        ...
        writer_l.add_scalar('training/loss', dqn_loss, self.count) # 使用tensotboard画图
...
for k in range(10):
    if args.env == 'sc':
        ...
        env = EnvMV()
    env.att_num = args.attacker
    random.seed()

    string = str(k+1)
    title = args.env + "-" + args.attacker + "("+ string + ")"
    writer_l = SummaryWriter('./log1/log_{}/loss'.format(title))
    writer_r = SummaryWriter('./log1/log_{}/return'.format(title)) 
    ...
    state_dim = env.observation_space.shape[0] * env.observation_space.shape[1] # 状态空间维度
    action_dim = env.action_space.n # 动作空间维度
    agent = DQN(state_dim, hidden1_dim, hidden2_dim, action_dim, lr, gamma, epsilon,
                target_update, device) # dqn算法参数初始化
        ...
        with tqdm(total=int(num_episodes / 10), desc='Iteration %d' % i) as pbar:
            for i_episode in range(int(num_episodes / 10)):
                ...
                while not terminated and not truncated:
                    ...
                    replay_buffer.add(state.reshape((1,state_dim)), action, reward, next_state.reshape((1,state_dim)), terminated)
                    state = next_state
                ...
                test_episode = 10
                if (i_episode) % 10 == 0: # 每10轮游戏，进行测试。测试10轮游戏的平均值
                    test_return_list = []
                    for j in range(test_episode):
                        ...
                        while not test_terminated and not test_truncated:
                            test_action = agent.predict(test_state.reshape((1,state_dim)))
                            test_action_list.append(test_action)
                            test_next_state, test_reward, test_terminated, test_truncated, test_info = env.step(test_action)
                    ...
                    write.writerow({'episode':num_episodes / 10 * i + i_episode,'avg_return':avg_test_return})
                    writer_r.add_scalar('training/return', avg_test_return, num_episodes / 10 * i + i_episode)
                
                
                if (i_episode + 1) % 10 == 0: # 每10轮游戏，计算平均回报
                    pbar.set_postfix({
                        'episode':
                        '%d' % (num_episodes / 10 * i + i_episode + 1), # 代表总的轮次
                        'return':
